...
export const FAMILY_EDGES: GraphEdge[] = [
  ...
  { source: 1, target: 5, label: 'Father → Daughter (Aisha)', strength: 1 },
  {
    source: 1,
    target: 2,
    label: "Father-in-law of Umar (Asma married Umar's brother)",
    strength: 0.6,
  },
  // Umar family
  {
    source: 2,
    target: 5,
    label: "Umar's daughter Hafsa = wife of Prophet ﷺ — Aisha is step-wife relation",
    strength: 0.5,
  },
  // Uthman family (married two daughters of Prophet ﷺ)
  { source: 3, target: 4, label: "Both married into Prophet's ﷺ family", strength: 0.5 },
  // Ali family
  { source: 4, target: 5, label: 'Aisha + Ali — both beloved by Prophet ﷺ', strength: 0.4 },
  { source: 4, target: 7, label: 'Ali + Hamza — cousins', strength: 0.9 },
  { source: 4, target: 6, label: "Ali + Zayd — both raised in Prophet's ﷺ home", strength: 0.7 },
  // Hamza
  { source: 7, target: 1, label: "Hamza is milk-uncle of Abu Bakr's daughter", strength: 0.4 },
  // Khadija / Prophet's family circle
  { source: 5, target: 19, label: 'Aisha + Ibn Abbas — uncle-niece relationship', strength: 0.8 },
  // Anas ibn Malik family
  {
    source: 13,
    target: 5,
    label: "Anas's half-sister Abu Talha married — extended Ansar family",
    strength: 0.4,
  },
  // Bilal + Abu Bakr (freed slave — brotherhood bond)
  { source: 10, target: 1, label: 'Freed by Abu Bakr — Islamic brotherhood', strength: 0.8 },
  // Sa'd ibn Abi Waqqas — maternal uncle of Prophet ﷺ
  { source: 8, target: 4, label: 'Both of Hashim lineage — tribal cousins', strength: 0.6 },
  // Salman al-Farisi — adopted into Prophet's family as "Ahl al-Bayt" (hadith)
  { source: 29, target: 4, label: 'Prophet ﷺ declared Salman Ahl al-Bayt', strength: 0.7 },
  // Abu Ubayda — no direct blood but very close
  { source: 9, target: 1, label: 'Abu Ubayda + Abu Bakr — closest inner circle', strength: 0.6 },
  // Abd al-Rahman ibn Awf — married Abu Bakr's daughter
  { source: 11, target: 1, label: "Married Abu Bakr's daughter Umm Kulthum", strength: 0.8 },
  ...
  { source: 20, target: 6, label: 'Usama ibn Zayd — son of Zayd (rank 6)', strength: 1 },
  {
    source: 20,
    target: 5,
    label: 'Usama + Aisha — step-relationship through Prophet ﷺ family',
    strength: 0.5,
  },
  // Asma bint Umays — wife of Ja'far, then Abu Bakr
  {
    source: 40,
    target: 1,
    label: "Asma bint Umays married Abu Bakr after Ja'far's martyrdom",
    strength: 0.8,
  },
  // Khalid + Umar — allied by conquest
  {
    source: 12,
    target: 2,
    label: 'Khalid commanded by Umar; Umar eventually dismissed him',
    strength: 0.5,
  },
  // Abdullah ibn Umar (rank ~30) + Umar
  { source: 30, target: 2, label: 'Abdullah ibn Umar — son of Umar', strength: 1 },
  { source: 30, target: 5, label: 'Ibn Umar + Aisha — shared many narrations', strength: 0.5 },
];
...
export const TRADE_EDGES: GraphEdge[] = [
  {
    source: 1,
    target: 11,
    label: 'Abu Bakr + Abd al-Rahman — major trading partners',
    strength: 0.9,
  },
  {
    source: 1,
    target: 3,
    label: 'Abu Bakr + Uthman — both Quraysh textile merchants',
    strength: 0.8,
  },
  {
    source: 3,
    target: 11,
    label: 'Uthman + Abd al-Rahman — commercial alliance & market co-founders',
    strength: 0.9,
  },
  {
    source: 2,
    target: 11,
    label: 'Umar + Abd al-Rahman — Umar relied on Ibn Awf for state economics',
    strength: 0.6,
  },
  { source: 11, target: 8, label: "Abd al-Rahman + Sa'd — traded routes to Syria", strength: 0.7 },
  {
    source: 7,
    target: 1,
    label: 'Hamza + Abu Bakr — trading connections pre-Islam',
    strength: 0.5,
  },
  {
    source: 37,
    target: 3,
    label: 'Abu Sufyan + Uthman — Umayyad commercial alliance',
    strength: 0.7,
  },
  {
    source: 37,
    target: 11,
    label: 'Abu Sufyan + Abd al-Rahman — both major Quraysh traders',
    strength: 0.6,
  },
  {
    source: 9,
    target: 11,
    label: 'Abu Ubayda + Ibn Awf — co-founders of Medina market structure',
    strength: 0.6,
  },
  { source: 11, target: 12, label: 'Ibn Awf + Khalid — financed early conquests', strength: 0.5 },
  {
    source: 29,
    target: 8,
    label: "Salman + Sa'd — established trade in Iraq after conquest",
    strength: 0.6,
  },
  {
    source: 19,
    target: 3,
    label: "Ibn Abbas + Uthman — Ibn Abbas was close to Uthman's caliphate",
    strength: 0.4,
  },
];
...
export const IKHTILAF_EDGES: GraphEdge[] = [
  // Abu Bakr vs Umar — Ridda wars (Abu Bakr fought apostates; Umar initially disagreed)
  {
    source: 1,
    target: 2,
    label: 'Ridda Wars: Abu Bakr decided to fight; Umar initially opposed',
    strength: 0.7,
  },
  // Umar vs Aisha — many juristic differences
  {
    source: 2,
    target: 5,
    label: "Mut'a marriage (Umar abolished; Aisha narrated different rulings)",
    strength: 0.8,
  },
  // Umar vs Ali — succession / political differences
  {
    source: 2,
    target: 4,
    label: 'Succession after Prophet ﷺ: Ali believed he had prior right; Umar chose Abu Bakr',
    strength: 0.9,
  },
  // Ali vs Aisha — Battle of Jamal
  {
    source: 4,
    target: 5,
    label: 'Battle of the Camel (36 AH): political / military confrontation',
    strength: 1,
  },
  // Ibn Abbas vs Zayd ibn Thabit — inheritance calculations
  {
    source: 19,
    target: 18,
    label: "Inheritance ('awl): Ibn Abbas rejected the 'awl doctrine; Zayd supported it",
    strength: 0.8,
  },
  // Ibn Abbas vs Umar — temporary marriage and finger-cut theft
  {
    source: 19,
    target: 2,
    label: 'Multiple juristic differences: wine prohibition wording, pilgrimage rites',
    strength: 0.7,
  },
  // Ibn Masud vs others — Quran compilation (opposed Uthman\'s burning of variant copies)
  {
    source: 22,
    target: 3,
    label: 'Ibn Masud refused to surrender his personal mushaf; opposed Uthmani compilation',
    strength: 0.9,
  },
  {
    source: 22,
    target: 18,
    label: 'Ibn Masud vs Zayd: disputed some Quranic order and surah inclusions',
    strength: 0.7,
  },
  // Abu Dharr vs Uthman — wealth distribution (Abu Dharr was anti-wealth accumulation)
  {
    source: 15,
    target: 3,
    label: 'Abu Dharr publicly criticized the wealth of Uthman and companions — exiled to Rabadha',
    strength: 1,
  },
  {
    source: 15,
    target: 11,
    label: 'Abu Dharr vs Ibn Awf: openly condemned commercial wealth as forbidden hoarding',
    strength: 0.8,
  },
  // Muawiyah vs Ali — Siffin and Quran arbitration
  {
    source: 4,
    target: 42,
    label: 'Battle of Siffin (37 AH): Ali vs Muawiyah on the caliphate legitimacy',
    strength: 1,
  },
  // Abu Hurayra vs Umar — number of hadiths narrated (Umar limited hadith narration)
  {
    source: 17,
    target: 2,
    label: 'Umar restricted excessive hadith narration; Abu Hurayra defied this in practice',
    strength: 0.7,
  },
  // Ammar vs Uthman — political opposition
  {
    source: 27,
    target: 3,
    label: "Ammar ibn Yasir opposed Uthman's policies and was beaten by his supporters",
    strength: 0.8,
  },
  // Khalid vs Umar — Khalid killed an ally (incident at Malik ibn Nuwayra); Umar demanded punishment
  {
    source: 12,
    target: 2,
    label: 'Killing of Malik ibn Nuwayra: Umar demanded Khalid be punished; Abu Bakr pardoned',
    strength: 0.8,
  },
];
...
export const CITY_POSITIONS: Record<string, CityPosition> = {
  Mecca: { name: 'Mecca', x: 0.52, y: 0.6, label: 'مكة المكرمة' },
  Medina: { name: 'Medina', x: 0.5, y: 0.52, label: 'المدينة المنورة' },
  Taif: { name: 'Taif', x: 0.53, y: 0.63 },
  "Mada'in": { name: "Mada'in", x: 0.63, y: 0.38 },
  Damascus: { name: 'Damascus', x: 0.44, y: 0.32 },
  Jerusalem: { name: 'Jerusalem', x: 0.41, y: 0.36 },
  Cairo: { name: 'Cairo/Egypt', x: 0.33, y: 0.4 },
  Kufa: { name: 'Kufa', x: 0.61, y: 0.4 },
  Basra: { name: 'Basra', x: 0.64, y: 0.46 },
  Baghdad: { name: 'Baghdad', x: 0.62, y: 0.36 },
  Persia: { name: 'Persia/Isfahan', x: 0.72, y: 0.34 },
  Abyssinia: { name: 'Abyssinia', x: 0.48, y: 0.76 },
  Constantinople: { name: 'Constantinople', x: 0.38, y: 0.2 },
  Yemen: { name: "San'a (Yemen)", x: 0.54, y: 0.73 },
  Najd: { name: 'Najd', x: 0.56, y: 0.55 },
  Syria: { name: 'Syria (Sham)', x: 0.44, y: 0.3 },
  Iraq: { name: 'Iraq', x: 0.62, y: 0.4 },
  Najaf: { name: 'Najaf', x: 0.61, y: 0.43 },
  Hims: { name: 'Homs (Hims)', x: 0.43, y: 0.28 },
  Alexandria: { name: 'Alexandria', x: 0.31, y: 0.38 },
  Tunisia: { name: 'North Africa', x: 0.22, y: 0.32 },
  Andalus: { name: 'Al-Andalus', x: 0.12, y: 0.28 },
  Bahrain: { name: 'Bahrain', x: 0.65, y: 0.53 },
  Makran: { name: 'Makran (Pakistan)', x: 0.78, y: 0.48 },
  Khurasan: { name: 'Khurasan', x: 0.78, y: 0.32 },
  Rabadha: { name: 'Rabadha', x: 0.55, y: 0.58 },
  Azruh: { name: 'Azruh (Jordan)', x: 0.43, y: 0.37 },
};
...
export const KEY_EVENTS: KeyEvent[] = [
  {
    label: 'First Revelation (0 AH / 610 CE)',
    yearAH: 0,
    note: 'Surah Al-Alaq revealed at Cave Hira',
  },
  {
    label: 'Hijra — Migration to Medina (1 AH)',
    yearAH: 1,
    note: 'Foundation of the Islamic state',
  },
  ...
  {
    label: 'Year of the Congregation (41 AH)',
    yearAH: 41,
    note: 'Hasan transfers caliphate to Muawiyah',
  },
  {
    label: 'Death of Muawiyah (60 AH)',
    yearAH: 60,
    note: 'Yazid takes power; Husayn refuses bayah',
  },
...
export const MIGRATION_PATHS: Record<number, MigrationStep[]> = {
  ...
  4: [
    {
      city: 'Mecca',
      yearAH: -22,
      note: "Born in Ka'ba — first birth inside Ka'ba in Islamic history",
    },
  ...
  10: [
    ...
    { city: 'Medina', yearAH: 1, note: 'Freed by Abu Bakr; first muezzin' },
    {
      city: 'Damascus',
      yearAH: 12,
      note: "Moved to Syria after Prophet's ﷺ death; gave final adhan there",
    },
  ...
  22: [
    ...
    { city: 'Kufa', yearAH: 23, note: 'Sent to Kufa by Umar to teach Quran; stayed 20+ years' },
    {
      city: 'Medina',
      yearAH: 32,
      note: 'Recalled by Uthman; died in Medina refusing to give mushaf',
    },
  ],
  29: [
    ...
    { city: 'Syria', yearAH: -20, note: '15 years serving Christian monks across Syria' },
    {
      city: 'Medina',
      yearAH: 5,
      note: 'Arrived in Medina; converted; suggested the Trench tactic',
    },
    { city: "Mada'in", yearAH: 21, note: "Appointed governor of al-Mada'in (Ctesiphon) by Umar" },
    {
      city: "Mada'in",
      yearAH: 36,
      note: "Died in Mada'in; one of the greatest companions of non-Arab origin",
    },
  ],
};
...
export const TEACHER_STUDENT_EDGES: TeacherStudentEdge[] = [
  ...
  {
    teacherRank: 1,
    studentName: 'Urwa ibn al-Zubayr',
    studentAr: 'عروة بن الزبير',
    subject: 'Sira and early Islamic history',
    legacy:
      "Became the first systematic biographer — his accounts form the backbone of Ibn Hisham's Seerah",
  },
  {
    teacherRank: 1,
    studentName: 'Qasim ibn Muhammad',
    studentAr: 'القاسم بن محمد',
    subject: 'Fiqh and caliphate history',
    legacy: "One of the Seven Jurists of Medina; transmitted Abu Bakr's legal positions",
  },
  // Umar's transmission line
  {
    teacherRank: 2,
    studentName: "Nafi' mawla Ibn Umar",
    studentAr: 'نافع مولى ابن عمر',
    subject: 'Sunnah and Medina practice',
    legacy: "The 'Golden Chain': Malik←Nafi'←Ibn Umar←Prophet ﷺ — most authenticated isnad",
  },
  {
    teacherRank: 2,
    studentName: 'Said ibn al-Musayyab',
    studentAr: 'سعيد بن المسيب',
    subject: 'Fiqh and companion opinions',
    legacy: "Greatest tabi'i jurist — his rulings form the foundation of Maliki fiqh",
  },
  {
    teacherRank: 2,
    studentName: 'Masruq ibn al-Ajdad',
    studentAr: 'مسروق بن الأجدع',
    subject: 'Hadith and Quran interpretation',
    legacy: "Primary transmitter of Umar's legal opinions and Aisha's hadiths",
  },
  // Uthman's transmission line
  {
    teacherRank: 3,
    studentName: "Sa'id ibn al-Musayyab",
    studentAr: 'سعيد بن المسيب',
    subject: "Quran compilation and Uthman's rulings",
    legacy: "Preserved Uthman's fatawa — including the authoritative Uthmanic mushaf decisions",
  },
  // Ali's transmission line
  {
    ...
    studentName: 'Masruq ibn al-Ajdad',
    studentAr: 'مسروق',
    subject: "Ali's jurisprudence",
    legacy: "Transmitted Ali's fatawa on prayer, blood money, and governance",
  },
  {
    teacherRank: 4,
    studentName: 'Shuraih al-Qadi',
    studentAr: 'شريح القاضي',
    subject: 'Islamic judicial procedure',
    legacy:
      'First professional Islamic judge; trained by Ali — shaped all subsequent judicial tradition',
  },
  // Aisha's transmission line
  {
    ...
    studentName: 'Urwa ibn al-Zubayr',
    studentAr: 'عروة بن الزبير',
    subject: '2,210 hadiths across all topics',
    legacy:
      "Urwa was Aisha's nephew — took ALL her narrations; became the channel for half of Islamic fiqh",
  },
  {
    teacherRank: 5,
    studentName: 'Amra bint Abd al-Rahman',
    studentAr: 'عمرة بنت عبد الرحمن',
    subject: "Women's fiqh and family law",
    legacy:
      "Primary female tabi'iyya for Aisha's rulings on women — Imam Malik cited her constantly",
  },
  {
    teacherRank: 5,
    studentName: 'Masruq ibn al-Ajdad',
    studentAr: 'مسروق',
    subject: "Aisha's hadith narrations",
    legacy: 'Masruq and Urwa together transmitted ~90% of what we know from Aisha',
  },
  // Ibn Abbas's transmission line
  {
    teacherRank: 19,
    studentName: 'Ata ibn Abi Rabah',
    studentAr: 'عطاء بن أبي رباح',
    subject: 'Quran tafsir and Hajj rituals',
    legacy:
      'Greatest mufassir after Ibn Abbas; his tafsir became the base for all subsequent commentary',
  },
  {
    teacherRank: 19,
    studentName: 'Mujahid ibn Jabr',
    studentAr: 'مجاهد بن جبر',
    subject: 'Quran interpretation',
    legacy:
      'Mujahid read the Quran with Ibn Abbas 30 times — his tafsir is cited by al-Bukhari 300+ times',
  },
  {
    teacherRank: 19,
    studentName: 'Ikrima mawla Ibn Abbas',
    studentAr: 'عكرمة مولى ابن عباس',
    subject: 'Tafsir and hadith',
    legacy:
      "Transmitted Ibn Abbas's entire hadith corpus — became foundation of fiqh in North Africa",
  },
  {
    teacherRank: 19,
    studentName: "Sa'id ibn Jubayr",
    studentAr: 'سعيد بن جبير',
    subject: 'Quran sciences',
    legacy:
      "Martyred by al-Hajjaj — one of the most revered tabi'i scholars; his tafsir is preserved",
  },
  // Ibn Masud's transmission line
  {
    teacherRank: 22,
    studentName: "Alqama al-Nakha'i",
    studentAr: 'علقمة النخعي',
    subject: "Hadith and Ibn Masud's fiqh",
    legacy:
      "Ibn Masud's primary student — teacher of Ibrahim al-Nakha'i — teacher of Hammad — teacher of Abu Hanifa: the Hanafi chain",
  },
  {
    teacherRank: 22,
    studentName: 'Masruq ibn al-Ajdad',
    studentAr: 'مسروق',
    subject: 'Quran recitation methodology',
    legacy:
      "Transmitted Ibn Masud's unique recitation style — one of the most honored readers in early Islam",
  },
  // Anas ibn Malik's transmission line
  {
    teacherRank: 13,
    studentName: 'al-Hasan al-Basri',
    studentAr: 'الحسن البصري',
    subject: 'Prophetic ethics and zuhd',
    legacy:
      "Greatest preacher of the tabi'in generation; his sermons shaped Islamic spiritual tradition for centuries",
  },
  {
    teacherRank: 13,
    studentName: 'Muhammad ibn Sirin',
    studentAr: 'محمد بن سيرين',
    subject: 'Dream interpretation + hadith',
    legacy:
      'The foremost expert on dream interpretation in Islamic tradition — his work used to this day',
  },
  {
    teacherRank: 13,
    studentName: 'Thabit al-Bunani',
    studentAr: 'ثابت البناني',
    subject: 'Daily prophetic behavior',
    legacy: "Preserved the most intimate narrations about the Prophet's ﷺ daily routine",
  },
  // Abu Hurayra's transmission line
  {
    teacherRank: 17,
    studentName: 'Hammam ibn Munabbih',
    studentAr: 'همام بن منبه',
    subject: '5,374 hadiths directly',
    legacy: 'Wrote the Sahifah Hammam — the oldest dated hadith manuscript in existence (30 AH)',
  },
  {
    teacherRank: 17,
    studentName: 'Ibn al-Musayyab',
    studentAr: 'ابن المسيب',
    subject: 'Hadith and fiqh',
    legacy:
      "Abu Hurayra's son-in-law — took his entire collection; became the bridge to Maliki fiqh",
  },
  // Sa'd ibn Abi Waqqas's line
  {
    teacherRank: 8,
    studentName: "Aamir ibn Sa'd",
    studentAr: 'عامر بن سعد',
    subject: "Father's hadiths and battles",
    legacy: "Primary transmitter of Sa'd's narrations — many Sahih hadiths pass through him",
  },
];
