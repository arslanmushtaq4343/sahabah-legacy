...
function AhadithTab() {
  ...
  const loveLabelCountStr =
    loveLabelCount == null
      ? ''
      : lang === 'ur'
        ? new Intl.NumberFormat('ur-PK').format(loveLabelCount)
        : String(loveLabelCount);
  const loveLabel = loveLabelCountStr
    ? `${loveLabelStripped} (${loveLabelCountStr})`
    : loveLabelStripped;
    ...
    <div className={styles.ahadithTab}>
      <div className={styles.ahadithHero} style={{ '--ah-col': col.color } as React.CSSProperties}>
        <div className={styles.ahadithHeroInner}>
          ...
          <div className={styles.ahadithHeroSub}>{t('ahadith.librarySub')}</div>
        </div>
      </div>
          ...
          {mode === 'imam' &&
            collections.map(c => (
              <button
                ...
                type="button"
                className={[
                  styles.ahadithImamCard,
                  imamKey === c.imamKey ? styles.ahadithImamCardActive : '',
                ].join(' ')}
                onClick={() => {
                  setImamKey(c.imamKey);
                  setQ('');
                  setOpen({});
                }}
                ...
                title={`${c.imamName} — 20`}
              >
                <span className={styles.ahadithImamWatermark} aria-hidden="true">
                  20
                </span>
        ...
        <div className={styles.ahCmdBar}>
          ...
          <button
            ...
            onClick={() => {
              ...
              (mode === 'imam' ? filteredImam : filteredLove).forEach(h => {
                next[h.id] = true;
              });
              setOpen(next);
            }}
      ...
      <div className={styles.ahList}>
              ...
              >
                {tag === 'all'
                  ...
                  : tag === 'love'
                    ? t('ahadith.tags.love')
                    : tag === 'respect'
                      ? t('ahadith.tags.respect')
                      : tag === 'brotherhood'
                        ? t('ahadith.tags.brotherhood')
                        : tag === 'parents'
                          ? t('ahadith.tags.parents')
                          : tag === 'prophet'
                            ? t('ahadith.tags.prophet')
                            : t('ahadith.tags.kids')}
              </button>
            ))}
          </div>
        ...
        {(mode === 'imam' ? filteredImam : filteredLove).map((h, idx) => {
          ...
          return (
            <div
              key={h.id}
              className={styles.ahRow}
              style={{ '--had-col': col.color } as React.CSSProperties}
            >
              <button
                type="button"
              ...
              >
                ...
                <span
                  className={[styles.ahRowChevron, isOpen ? styles.ahRowChevronOpen : ''].join(' ')}
                  aria-hidden="true"
                />
              </button>
              {isOpen && (
                <div className={styles.ahRowBody}>
                  ...
                  {mode === 'love' && lang === 'ur' && h.ur && (
                    <div className={styles.ahRowUr}>{h.ur}</div>
                  )}
                    ...
                    <div className={styles.ahRowActions}>
                      ...
                      {(mode === 'love' || mode === 'imam') && h.ur && (
                        <button
                          type="button"
                          className={styles.ahRowActionBtn}
                          onClick={() => copyText(h.ur)}
                          title="Copy Urdu"
                          aria-label="Copy Urdu"
                        >
                          UR
                        </button>
                      )}
...
function JarhTadilSection({ imamKey }: { imamKey: string }) {
  ...
  const RATING_EN = {
    ...
    sadouq: 'Truthful (Sadouq)',
    'layyinul-hadith': 'Weak narrator',
    daif: 'Weak (Daif)',
    majhul: 'Unknown',
    hasan: 'Good (Hasan)',
  };
      ...
      {entries.map((e, i) => (
        <div key={i} className={styles.jtEntry}>
          <div className={styles.jtHeader}>
            ...
            <span
              className={styles.jtRating}
              style={{
                background: RATING_COLORS[e.rating] + '22',
                color: RATING_COLORS[e.rating],
                border: `1px solid ${RATING_COLORS[e.rating]}44`,
              }}
            >
          ...
          {e.specialStatus && <div className={styles.jtSpecial}>{e.specialStatus}</div>}
        </div>
      ))}
    </div>
  );
...
function ImamModal({
  ...
  const [openHadith, setOpenHadith] = useState<Record<string, boolean>>(() => {
    ...
    const next: Record<string, boolean> = {};
    hadithCollection.hadiths.forEach(h => {
      next[h.id] = true;
    });
    return next;
  });
  ...
  return (
    <div className={styles.overlay} onClick={e => e.target === e.currentTarget && onClose()}>
      <div className={styles.imamModal}>
        <button className={styles.imamModalClose} onClick={onClose} aria-label="Close">
          ×
        </button>
        <div className={styles.imamModalHeader} style={{ borderColor: col }}>
          <div className={styles.imamNum} style={{ background: IMAM_BG[imam.key], color: col }}>
            {imam.num}
          </div>
          <div>
            <p className={styles.imamModalAr}>{imam.ar}</p>
            <p className={styles.imamModalUr} style={{ color: col }}>
              {imam.ur}
            </p>
            <h2 className={styles.imamModalName}>{norm(imam.name)}</h2>
            <span
              className={styles.honorific}
              style={{ background: IMAM_BG[imam.key], color: col, borderColor: col + '60' }}
            >
              {imam.honorific}
            </span>
          </div>
        ...
        <div className={styles.imamModalBody}>
          ...
          <div className={styles.imamSection}>
            <h4>{lang === 'ur' ? 'ابتدائی حالات' : 'Origin & Early Life'}</h4>
            <p>{lang === 'ur' ? (imam.originUr ?? imam.origin) : imam.origin}</p>
            <p className={styles.mt4}>
              {lang === 'ur' ? (imam.heritageUr ?? imam.heritage) : imam.heritage}
            </p>
          </div>
          <div className={styles.imamSection}>
            <h4>{lang === 'ur' ? 'سلسلۂ اساتذہ' : 'Chain to the Prophet ﷺ'}</h4>
            ...
            <p className={styles.mt4}>
              <strong>{lang === 'ur' ? 'اساتذہ:' : 'Full teachers:'}</strong>{' '}
              {lang === 'ur' ? (imam.teachersUr ?? imam.teachers) : imam.teachers}
            </p>
          </div>
          <div className={styles.imamSection}>
            <h4>{lang === 'ur' ? 'مشہور شاگرد' : 'Famous Students'}</h4>
            <p>{lang === 'ur' ? (imam.studentsUr ?? imam.students) : imam.students}</p>
          </div>
          <div className={styles.imamSection}>
            <h4>{lang === 'ur' ? 'کتب و وراثت' : 'Key Works & Legacy'}</h4>
            <p>{lang === 'ur' ? (imam.keyWorksUr ?? imam.keyWorks) : imam.keyWorks}</p>
            <p className={styles.mt4}>
              {lang === 'ur' ? (imam.books_legacyUr ?? imam.books_legacy) : imam.books_legacy}
            </p>
          </div>
          <div className={styles.imamSection}>
            <h4>{lang === 'ur' ? 'فقہی طریقہ (اصول)' : 'Legal Methodology'}</h4>
            <div className={styles.methodBox} style={{ borderColor: col }}>
              <p>{lang === 'ur' ? (imam.methodUr ?? imam.method) : imam.method}</p>
            </div>
          </div>
          <div className={styles.imamSection}>
            <h4>{lang === 'ur' ? 'آزمائش و اصول' : 'Trial & Principles'}</h4>
            <p>{lang === 'ur' ? (imam.trialUr ?? imam.trial) : imam.trial}</p>
          </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        <span
                          className={[
                            styles.hadithChevron,
                            isOpen ? styles.hadithChevronOpen : '',
                          ].join(' ')}
                          aria-hidden="true"
                        />
                      </button>
...
/* ═══════════════════════════════════════════════════════
   TAB 1 · FOUR IMAMS
   ═══════════════════════════════════════════════════════ */
function FourImamsTab({ openImamId }: { openImamId?: string }) {
  const [selected, setSelected] = useState<Imam | null>(null);
  const [autoOpenAhadith, setAutoOpenAhadith] = useState(false);
  const { lang } = useLanguage();
  const { toggleImam, isImamSelected } = useCompare();

  useEffect(() => {
    if (!openImamId) return;
    const imam = IMAMS.find(im => im.id === openImamId || im.key === openImamId);
    if (!imam) return;
    setSelected(imam);
    setAutoOpenAhadith(false);
  }, [openImamId]);
    ...
    <div>
      <div className={styles.imamGrid}>
        {IMAMS.map(im => {
          ...
          const compared = isImamSelected(im.id);
          return (
            <div
              key={im.id}
              className={styles.imamCard}
              style={
                {
                  '--imam-color': col,
                  '--imam-bg': IMAM_BG[im.key],
            ...
            >
              <div className={styles.imamCardTop}>
                <div className={styles.imamTopRow}>
                  <div className={styles.imamBadge}>
                    {im.num}
                  </div>
                  <div>
                    <p className={styles.imamCardAr}>
                      {im.ar}
                    </p>
                    <p className={styles.imamCardUr}>
                      {im.ur}
                    </p>
                    <h3 className={styles.imamCardName}>{norm(im.name)}</h3>
                    <span className={styles.honorific}>
                      {im.honorific}
                    </span>
                  </div>
                </div>
                <div className={styles.imamQuickActions}>
                  <button
                    type="button"
                    className={styles.imamQuickBtn}
                    onClick={e => {
                      e.stopPropagation();
                      speakArabic(im.ar);
                    }}
                    title="Hear Arabic pronunciation"
                    aria-label={`Hear Arabic pronunciation for ${norm(im.name)}`}
                  >
                    Name audio
                  </button>
                  <button
                    ...
                    onClick={e => {
                      e.stopPropagation();
                      speakBio(im.name, `${im.honorific}. ${im.sig}`);
                    }}
                    title="Listen to short biography"
                    aria-label={`Listen to short biography for ${norm(im.name)}`}
                  >
                    Bio audio
                  </button>
                  <button
                    type="button"
                    className={`${styles.imamQuickBtn} ${compared ? styles.imamQuickBtnActive : ''}`}
                    onClick={e => {
                      e.stopPropagation();
                      toggleImam(im.id);
                    }}
                ...
                <p className={styles.imamDates}>
                  {im.born}, {im.bornPlace} &nbsp;·&nbsp; {im.died}, {im.diedPlace}
                </p>
              </div>
              <div className={styles.imamCardBody}>
                ...
                <div className={styles.imamGrid2}>
                  <div className={styles.igItem}>
                    <span className={styles.igL}>{lang === 'ur' ? 'اصل' : 'Origin'}</span>
                    <span className={styles.igV}>
                      {(lang === 'ur' ? (im.originUr ?? im.origin) : im.origin).split('.')[0]}
                    </span>
                  </div>
                  <div className={styles.igItem}>
                    <span className={styles.igL}>{lang === 'ur' ? 'اثر' : 'Global Reach'}</span>
                    <span className={styles.igV}>
                      {(lang === 'ur' ? (im.reachUr ?? im.reach) : im.reach).split('.')[0]}
                    </span>
                  </div>
                  <div className={styles.igItem}>
                    <span className={styles.igL}>{lang === 'ur' ? 'اہم کتاب' : 'Key Work'}</span>
                    <span className={styles.igV}>
                      {(lang === 'ur' ? (im.keyWorksUr ?? im.keyWorks) : im.keyWorks)
                        .split('.')[0]
                        .split('?')[0]
                        .trim()}
                    </span>
                  ...
                  <div className={styles.igItem}>
                    <span className={styles.igL}>
                      {lang === 'ur' ? 'مشہور شاگرد' : 'Top Student'}
                    </span>
                    <span className={styles.igV}>
                      {(lang === 'ur' ? (im.studentsUr ?? im.students) : im.students)
                        .split('(')[0]
                        .split(',')[0]
                        .trim()}
                    </span>
                ...
                <div className={styles.methodBox}>
                  <p className={styles.methodTitle}>
                    {lang === 'ur' ? 'فقہی طریقہ (اصول)' : 'Legal Method (Usul)'}
                  </p>
                  <p className={styles.methodText}>
                    {(lang === 'ur' ? (im.methodUr ?? im.method) : im.method).substring(0, 160)}…
                  </p>
                </div>
              </div>
              ...
              <div className={styles.imamReachBar}>
                {norm(im.reach).split('.')[0]} &nbsp;·&nbsp; <strong>Click for full profile</strong>
                {hasHadith && (
                  <button
                    type="button"
      ...
      {selected && (
        <ImamModal
          ...
          initialAhadithOpen={autoOpenAhadith}
        />
      )}
    </div>
  );
      ...
      {eraOrder.map(eraKey => {
        ...
        return (
          <div
            ...
            className={[
              styles.eraRow,
              isSource ? styles.eraSource : '',
              isToday ? styles.eraToday : '',
            ].join(' ')}
          >
            <div className={styles.eraLabel}>
              <span className={styles.eraNum}>
                {isSource ? '★' : isToday ? '◉' : `Era ${era.id}`}
              </span>
              <span className={styles.eraName}>{era.label}</span>
              <span className={styles.eraYr}>{era.yr}</span>
            </div>
            <div className={styles.eraConnector}>
              ...
              {nodes.map(n => {
                ...
                return (
                  <div
                    ...
                    className={[
                      styles.chainNode,
                      n.IMAM ? styles.chainNodeImam : '',
                      n.special === 'today' ? styles.chainNodeToday : '',
                      n.special === 'golden' ? styles.chainNodeGolden : '',
                    ].join(' ')}
  ...
  return (
    <div>
      <div className={styles.isnadSubBar}>
        <button
          className={`${styles.isnadSubBtn} ${isnadView === 'chains' ? styles.isnadSubActive : ''}`}
          onClick={() => setIsnadView('chains')}
        >
          Isnad Chains
        </button>
        <button
          className={`${styles.isnadSubBtn} ${isnadView === 'final' ? styles.isnadSubActive : ''}`}
          onClick={() => setIsnadView('final')}
        >
          Final Circle
        </button>
        <button
          className={`${styles.isnadSubBtn} ${isnadView === 'builder' ? styles.isnadSubActive : ''}`}
          onClick={() => setIsnadView('builder')}
        >
          Chain Builder
        </button>
      </div>
          ...
          {ISNADS_DATA.map((isn, i) => (
            <div key={i} className={styles.isnadCard}>
              <div className={styles.isnadHdr}>
                <p className={styles.isnadNum}>
                  HADITH {i + 1} OF {ISNADS_DATA.length}
                </p>
              ...
              <div className={styles.isnadBody}>
                ...
                <p className={styles.isnadChainLbl}>
                  COMPLETE ISNAD FROM PROPHET TO YOU ({isn.chain.length} LINKS)
                </p>
                  ...
                  {isn.chain.map((nd, ni) => (
                    <div key={ni} className={styles.isnadStep}>
                      ...
                      <div className={styles.isnadNode}>
                        <div
                          className={styles.isnadCircle}
                          style={{ background: nd.c, color: nd.tc, borderColor: nd.c }}
                        >
                          {nd.ar.split(' ').slice(0, 2).join('\n')}
                        </div>
                        <p className={styles.isnadNname}>{nd.name}</p>
                        <p className={styles.isnadNrole}>{nd.yr}</p>
                        <p className={styles.isnadNrole}>
                          {nd.role.length > 40 ? nd.role.substring(0, 40) + '...' : nd.role}
                        </p>
                      </div>
                    </div>
                  ))}
                </div>
                <div className={styles.isnadNote}>
                  <strong>Chain Analysis:</strong> {isn.note}
                </div>
              </div>
            </div>
          ))}
      ...
      <div className={styles.fcCircle}>
        {fc.presentCompanions.map((c, i) => (
          <div key={i} className={styles.fcCompCard} style={{ borderTopColor: c.color }}>
            ...
            <div className={styles.fcCompName} style={{ color: c.color }}>
              {c.name}
            </div>
        ...
        {LEGACY_SCHOOLS.map(s => (
          <div
            ...
            style={{
              ...
              background: `linear-gradient(165deg,${s.bg},rgba(22,20,15,.99))`,
            }}
          >
            <div
              className={styles.schoolTop}
              style={{ background: `linear-gradient(135deg,${s.bg},${s.bg}88)` }}
            >
              <p className={styles.schoolPct} style={{ color: s.col }}>
                {s.pct}
              </p>
              <p className={styles.schoolPop} style={{ color: s.tc }}>
                {s.pop}
              </p>
              <h3 className={styles.schoolName} style={{ color: s.col }}>
                {s.name}
              </h3>
              <p className={styles.schoolAr} style={{ color: s.col }}>
                {s.ar}
              </p>
            </div>
            <div className={styles.schoolBody}>
              <p className={styles.schoolRegions} style={{ color: s.tc }}>
                <strong>Active in:</strong> {s.regions}
              </p>
              <p className={styles.schoolWhy} style={{ color: s.tc }}>
                <strong>Why this region?</strong> {s.why.substring(0, 180)}…
              </p>
              <div className={styles.schoolScholars}>
                {s.scholars.map(sc => (
                  <span
                    ...
                    style={{ color: s.col, borderColor: s.border }}
                  >
                    {sc}
                  </span>
                ))}
        ...
        <div className={styles.diasporaWrap}>
          ...
          <div className={styles.diasporaSvgWrap}>
            ...
            >
              ...
              {/* Region labels */}
              {[
                { label: 'ARABIA', x: 420, y: 195 },
                { label: 'PERSIA/IRAN', x: 530, y: 160 },
                { label: 'SOUTH ASIA', x: 620, y: 175 },
                { label: 'CENTRAL ASIA', x: 570, y: 115 },
                { label: 'NORTH AFRICA', x: 250, y: 155 },
                { label: 'WEST AFRICA', x: 155, y: 215 },
                { label: 'EUROPE', x: 230, y: 80 },
                { label: 'SE ASIA', x: 700, y: 235 },
              ].map(r => (
                <text
                  key={r.label}
                  x={r.x}
          ...
          <div className={styles.diasporaList}>
            {(['hf', 'ml', 'sf', 'hb'] as const).map(madhab => (
              <div key={madhab} className={styles.diasporaMadhab}>
                <div
                  className={styles.diasporaMadhabTitle}
                  style={{ color: MADHAB_COLORS[madhab] }}
                >
                ...
                {DIASPORA_CENTERS.filter(d => d.madhab === madhab).map((d, i) => (
                  <div key={i} className={styles.diasporaCity}>
                    <span
                      className={styles.diasporaCityDot}
                      style={{ background: MADHAB_COLORS[madhab] }}
                    />
        ...
        {MODERN_SCHOLARS.map(s => {
          ...
          return (
            <div key={s.name} className={styles.modernCard}>
              ...
              <span
                className={styles.madhab}
                style={{ background: bg, color: col, borderColor: col + '50' }}
              >
                {MADHAB_LABEL[s.key]}
              </span>
            </div>
...
function HouseholdTab() {
  ...
  return (
    <div>
      <h2 className={styles.sectionTitle}>The Prophet's Household - Those Who Served</h2>
      <div className={styles.hhIntro}>
        A complete register of companions who lived in or regularly served the Prophet's household -
        each a living witness to his private character.
      </div>
      <div className={styles.hhGrid}>
        {HOUSEHOLD_STAFF.map((m, i) => {
          ...
          return (
            <div
              key={i}
              className={`${styles.hhCard} ${expanded === i ? styles.hhCardOpen : ''}`}
            ...
            >
              <div className={styles.hhCardTop}>
                ...
                <span className={styles.hhCatBadge} style={{ background: col + '22', color: col }}>
                  {m.roleCategory.replace('-', ' ')}
                </span>
...
function SahabiyyatTab() {
  ...
  const filtered = SAHABIYYAT.filter(
    s =>
      (catFilter === 'all' || s.category === catFilter) &&
      (!search ||
        norm(s.name).toLowerCase().includes(search.toLowerCase()) ||
        s.nameAr.includes(search))
  );

  return (
    <div>
      <h2 className={styles.sectionTitle}>Sahabiyyat - Women of the First Generation</h2>
      <div className={styles.sabIntro}>
        The women companions of the Prophet - scholars, warriors, nurses, narrators, and the first
        martyrs. 200+ women are recorded; shown here are the most documented.
      </div>
      <div className={styles.sabControls}>
        <input
          className={styles.sabSearch}
        ...
        <div className={styles.sabCatBtns}>
          {cats.map(cat => (
            <button
              key={cat}
              className={`${styles.sabCatBtn} ${catFilter === cat ? styles.sabCatActive : ''}`}
              style={
        ...
        {cats
          ...
          .map(cat => (
            <div key={cat} className={styles.sabStat}>
              <span className={styles.sabStatNum} style={{ color: CAT_COLORS[cat] }}>
                {SAHABIYYAT.filter(s => s.category === cat).length}
              </span>
      ...
      <div className={styles.sabGrid}>
        {filtered.map((s, i) => {
          ...
          return (
            <div
              key={s.id}
              className={`${styles.sabCard} ${isOpen ? styles.sabCardOpen : ''}`}
              style={{ borderTopColor: col }}
              onClick={() => setSelected(isOpen ? null : s.id)}
            >
              <div className={styles.sabCardTop}>
                <div className={styles.sabNameAr}>{s.nameAr}</div>
                <div className={styles.sabName}>{norm(s.name)}</div>
                {s.title && (
                  <div className={styles.sabTitle} style={{ color: col }}>
                    {s.title}
                  </div>
                )}
                <div className={styles.sabMeta}>
                  <span
                    className={styles.sabCatBadge}
                    style={{ background: col + '22', color: col }}
                  >
                    {s.category}
                  </span>
                  {s.hadiths ? (
                    <span className={styles.sabHadiths}>{s.hadiths.toLocaleString()} hadiths</span>
                  ) : null}
...
function ConvertsTab() {
  ...
  return (
    <div>
      <h2 className={styles.sectionTitle}>Former Enemies - Conversion Stories</h2>
      <div className={styles.convIntro}>
        The most dramatic turnarounds in Islamic history — men and women who dedicated their lives
        to opposing Islam, then became its champions. The same people. A different heart.
      </div>
      {CONVERTS_DATA.map((c, i) => (
        <div
          key={i}
          className={`${styles.convCard} ${selected === i ? styles.convCardOpen : ''}`}
        ...
        >
          <div className={styles.convHeader} onClick={() => setSelected(selected === i ? null : i)}>
            <div className={styles.convLeft}>
              ...
              <div className={styles.convName} style={{ color: c.color }}>
                {c.name}
              </div>
          ...
          {selected === i && (
            <div className={styles.convBody}>
              <div className={styles.convSection}>
                <div className={styles.convSectionTitle} style={{ color: '#8b1a38' }}>
                  Before Islam
                </div>
                <p>{c.preIslamRole}</p>
              </div>
              <div className={styles.convSection}>
                <div className={styles.convSectionTitle} style={{ color: '#b8860b' }}>
                  The Turning Point
                </div>
                <p>{c.conversionTrigger}</p>
              </div>
              <div className={styles.convSection}>
                <div className={styles.convSectionTitle} style={{ color: c.color }}>
              ...
              <div className={styles.convSection}>
                <div className={styles.convSectionTitle} style={{ color: '#0a3d2e' }}>
                  Post-Islam Legacy
                </div>
...
function LaqabTab() {
  ...
  const filtered = LAQAB_DATA.filter(
    l =>
      !search ||
      norm(l.laqab).toLowerCase().includes(search.toLowerCase()) ||
      norm(l.laqabEn).toLowerCase().includes(search.toLowerCase()) ||
      norm(l.companion).toLowerCase().includes(search.toLowerCase()) ||
      l.laqabAr.includes(search)
  );

  return (
    <div>
      <h2 className={styles.sectionTitle}>Laqab - Prophetic Nicknames</h2>
      <div className={styles.laqabIntro}>
        Every honorific personally given by the Prophet - in the moment - to a companion. Each laqab
        is a divine snapshot of character, preserved in Arabic and transmitted through the
        centuries.
      </div>
      <div className={styles.laqabSearch}>
        <input
          className={styles.laqabInput}
          placeholder="Search by laqab, meaning, or companion name..."
          value={search}
          onChange={e => setSearch(e.target.value)}
        />
        <span className={styles.laqabCount}>
          {filtered.length} of {LAQAB_DATA.length} honorifics
        </span>
      </div>
      <div className={styles.laqabGrid}>
        {filtered.map((l, i) => {
          ...
          return (
            <div
              key={l.id}
              className={`${styles.laqabCard} ${isOpen ? styles.laqabCardOpen : ''}`}
            ...
            >
              <div className={styles.laqabCardTop}>
                ...
                <div className={styles.laqabName} style={{ color: l.color }}>
                  {norm(l.laqab)}
                </div>
                <div className={styles.laqabEn}>{norm(l.laqabEn)}</div>
                <div className={styles.laqabCompanion}>{norm(l.companion)}</div>
              </div>
              {isOpen && (
...
function GuideTab() {
  ...
  return (
    <div>
      <h2 className={styles.sectionTitle}>What Would They Say? Companion Hadith Guide</h2>
      <div className={styles.guideTabIntro}>
        Describe your situation. Surface the most relevant companions and their hadith narrations.
        Built from the site's own hadith corpus - direct quotes from those who witnessed the Prophet
        ﷺ.
      </div>
      <div className={styles.guideTabSearch}>
        ...
        />
        <button className={styles.guideTabBtn} onClick={search}>
          Find Hadiths
        </button>
      </div>
        ...
        <div className={styles.guideBrowse}>
          ...
          <div className={styles.guideSituationGrid}>
            {SITUATION_GUIDES.map((sg, i) => (
              ...
              >
                <span className={styles.guideSitCat} style={{ color: CAT_COLORS[sg.category] }}>
                  {sg.category}
                </span>
        ...
        <div>
          {results.length === 0 ? (
            <div className={styles.guideNoResults}>
              No results for "{query}". Try: grief, anger, money, family, knowledge, worship,
              leadership, decision.
            </div>
          ) : (
            results.map((sg, i) => (
      ...
      {selectedSit !== null && results === null && (
        <GuideResultBlock
          ...
          catColor={CAT_COLORS[SITUATION_GUIDES[selectedSit].category]}
        />
      )}
    </div>
  );
...
export default function ImamsPage() {
  ...
  const activeIdx = TABS.findIndex(ti => ti.key === tab);
  return (
    <div className={`${styles.page} premium-page`}>
      <header className={styles.atlasHead}>
        <div className={styles.atlasHeadTitleRow}>
          <h1 className={styles.atlasHeadTitle}>{t('imams.title')}</h1>
          <span className={styles.atlasHeadTitleAr} aria-hidden={lang === 'ur'}>
            {lang === 'ur' ? 'The Transmission Chain' : 'سِلْسِلَةُ الإِسْنَاد'}
          </span>
        </div>
        <p className={styles.atlasHeadSub}>{t('imams.subtitle')}</p>
        <div className={styles.atlasHeadStats}>
          <span className={styles.atlasHeadChip}>
            <span className={styles.atlasHeadChipDot} style={{ background: '#0f6f5c' }} aria-hidden="true" />
            <strong>4</strong>
            <span>{L('Madhhab Imams', 'ائمۂ مذاہب')}</span>
          </span>
          <span className={styles.atlasHeadChip}>
            <span className={styles.atlasHeadChipDot} style={{ background: '#b8860b' }} aria-hidden="true" />
            <strong>{TABS.length}</strong>
            <span>{L('Sections', 'سیکشنز')}</span>
          </span>
          <span className={styles.atlasHeadChip}>
            <span className={styles.atlasHeadChipDot} style={{ background: '#8b1a38' }} aria-hidden="true" />
            <strong>{activeIdx + 1}/{TABS.length}</strong>
            <span>{L('Viewing', 'دیکھ رہے ہیں')}</span>
          </span>
          <span className={styles.atlasHeadChip}>
            <span className={styles.atlasHeadChipDot} style={{ background: '#7a5530' }} aria-hidden="true" />
            <strong>1400+</strong>
            <span>{L('Years Preserved', 'سال محفوظ')}</span>
          </span>
        </div>
      </header>
      ...
      <div className={styles.tabContent}>
        {tab === 'imams' && <FourImamsTab openImamId={searchParams.get('imam') ?? undefined} />}
...
function IsnadChainBuilder() {
  ...
  const renderSelect = (nodes, val, setter, label, color) => (
    <div className={styles.ibSelectBox} style={{ borderColor: color + '44' }}>
      <div className={styles.ibSelectLabel} style={{ color }}>
        {label}
      </div>
      <select
        className={styles.ibSelect}
        value={val}
        onChange={e => {
  ...
  return (
    <div className={styles.ibPage}>
      <div className={styles.ibIntro}>
        Build your own isnad chain: choose a Sahabi, Tabi'i, and Classical Scholar. The system
        validates whether this chain is historically documented and shows the recorded chain for
        comparison.
      </div>
      ...
      <div className={styles.ibActions}>
        <button
          className={styles.ibValidateBtn}
          onClick={validate}
          disabled={!sel0 || !sel1 || !sel2}
        >
          Validate Chain
        </button>
        <button className={styles.ibResetBtn} onClick={reset}>
          Reset
        </button>
      </div>
      ...
      {validated && (
        <div
          className={styles.ibResult}
          style={{ borderColor: validated.isValid ? '#0a5c2e' : '#8b1a38' }}
        >
          {validated.isValid ? (
            <>
              <div className={styles.ibResultValid}>Historically Documented Chain</div>
              <p className={styles.ibResultNote}>
                This exact chain is recorded in classical hadith scholarship. The links are
                confirmed by multiple classical scholars.
              </p>
            </>
          ) : (
            <>
              ...
              <p className={styles.ibResultNote}>
                This specific combination is not recorded in our database. The narrators may have
                lived in the right eras, but there is no confirmed direct link in this exact order.
                Below are known valid chains for comparison.
              </p>
            </>
          )}
      ...
      <div className={styles.ibExplainer}>
        <strong>How Isnad Validation Works:</strong> Each link in a chain requires that the narrator
        (a) lived in the same era as their teacher, (b) resided in overlapping cities, and (c) is
        recorded as having met. Classical hadith scholars spent their lives verifying these
        connections - traveling across the Islamic world to confirm a single link.
      </div>
    </div>
  );
}
  ...
  return (
    <div className={styles.mrPage}>
      <p className={styles.mrIntro}>
        Enter any fiqh topic to see which companion's narration each madhab traces back to, the
        exact hadith, and why scholars interpreted it differently. Never built as an interactive
        tool before.
      </p>
          ...
          <div className={styles.mrNarrations}>
            ...
            {root.companionNarrations.map((n, i) => (
              <div key={i} className={styles.mrNarration}>
                <div className={styles.mrNarrComp}>
                  {n.companion} ? <em>{n.position}</em>
                </div>
          ...
          <div className={styles.mrMadhabs}>
            {root.madhabPositions.map((m, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`${styles.mrMadhab} ${activePos === i ? styles.mrMadhabActive : ''}`}
                  style={{ borderTopColor: st.color }}
                  onClick={() => setActivePos(activePos === i ? null : i)}
                >
                  <div className={styles.mrMadhabHeader}>
                    <span className={styles.mrMadhabName} style={{ color: st.color }}>
                      {m.madhab}
                    </span>
                  ...
                  {activePos === i && (
                    <p className={styles.mrMadhabReason}>
                      <strong>Reasoning:</strong> {m.reasoning}
                    </p>
                  )}
                </div>
              );
  ...
  return (
    <div className={styles.qrPage}>
      ...
      <p className={styles.qrIntro}>
        Each of the 7 canonical recitation styles (Qira'at Sab') traced back to the specific
        companion whose recitation it preserves. The oral tradition passed unbroken for 1,400 years
        through thousands of authenticated chains.
      </p>
      <div className={styles.qrPicker}>
        {QIRAAT_SEVEN.map(q => (
          <button
            ...
            style={
              selected.id === q.id
                ? { borderColor: q.color, background: q.color + '15', color: q.color }
                : {}
            }
      ...
      {selected && (
        <div className={styles.qrDetail} style={{ borderColor: selected.color }}>
          <div className={styles.qrDetailHeader} style={{ background: selected.color + '10' }}>
            ...
            <h3 className={styles.qrDetailName} style={{ color: selected.color }}>
              {selected.qariName}
            </h3>
            <div className={styles.qrDetailMeta}>
              <span>d. {selected.died}</span>
              <span
                className={styles.qrRegionBadge}
                style={{ borderColor: selected.color, color: selected.color }}
              >
                {selected.regionAr} ({selected.region})
              </span>
            </div>
          </div>
          <div className={styles.qrBody}>
            ...
            <div className={styles.qrExample}>
              <strong>Example verse: </strong>
              {selected.exampleVerse}
      ...
      <div className={styles.qrNote}>
        <strong>What makes Qira'at unique:</strong> These are not "translations" or "versions" —
        they are the same Quran with minor phonetic variations (vowel elongations, assimilation
        rules, dialect features) all traced back to the Prophet ﷺ through authenticated chains. Each
        style has been memorized and transmitted orally without interruption for 1,400 years.
      </div>
    </div>
  );
}
  ...
  return (
    <div className={styles.qsPage}>
      ...
      <p className={styles.qsIntro}>
        Companion opinions (qawl sahabi) adopted as binding or persuasive by subsequent classical
        scholars — with the usul al-fiqh debate around each, which madhab accepted it, and the exact
        companion statement.
      </p>
        ...
        {QAWL_SAHABI.map((q, i) => (
          ...
          >
            <div className={styles.qsCardTop}>
              <span className={styles.qsComp} style={{ color: q.color }}>
                {q.companion}
              </span>
              <span
                className={styles.qsStatus}
                style={{
                  background: STATUS_COLORS[q.usulStatus] + '22',
                  color: STATUS_COLORS[q.usulStatus],
                }}
              >
                {q.usulStatus}
              </span>
            ...
            {selected === i && (
              <div className={styles.qsDetail}>
                ...
                <p className={styles.qsUsulNote}>
                  <strong>Usul al-Fiqh note:</strong> {q.usulNote}
                </p>
                <div className={styles.qsMadhabs}>
                  {q.madhabAdoption.map((m, j) => (
                    <div
                      key={j}
                      className={styles.qsMadhab}
                      style={{ borderLeftColor: MADHAB_COLORS_Q[m.madhab] }}
                    >
                      <span
                        className={styles.qsMadhabName}
                        style={{ color: MADHAB_COLORS_Q[m.madhab] }}
                      >
                        {m.madhab}
                      </span>
                      <span
                        className={styles.qsMadhabVerdict}
                        style={{ color: m.accepted ? '#0a5c2e' : '#8b1a38' }}
                      >
  ...
  return (
    <div className={styles.ijPage}>
      ...
      <p className={styles.ijIntro}>
        Cases where a companion changed their own legal opinion after the Prophet ﷺ died — showing
        the living, growing nature of companion scholarship and intellectual honesty.
      </p>
      {IJTIHAD_CHANGES.map((c, i) => (
        <div
          key={i}
          className={styles.ijCard}
          style={{ borderLeftColor: c.color }}
          onClick={() => setExpanded(expanded === i ? null : i)}
        >
          <div className={styles.ijHeader}>
            <span className={styles.ijComp} style={{ color: c.color }}>
              {c.companion}
            </span>
            <span className={styles.ijTopic}>{c.topic}</span>
          </div>
          <div className={styles.ijPositions}>
            <div className={styles.ijOld}>
              <strong>Original:</strong> {c.originalPosition}
            </div>
            <div className={styles.ijArrow}>→ Changed to</div>
            <div className={styles.ijNew}>
              <strong>Revised:</strong> {c.revisedPosition}
            </div>
          </div>
          {expanded === i && (
            <>
              <p className={styles.ijReason}>
                <strong>Why:</strong> {c.reasonForChange}
              </p>
              <p className={styles.ijImpact}>
                <strong>Madhab impact:</strong> {c.madhahbImpact}
              </p>
