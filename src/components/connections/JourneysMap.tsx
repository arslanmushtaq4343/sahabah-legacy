...
const AVAILABLE_RANKS = Object.keys(MIGRATION_PATHS)
  .map(Number)
  .sort((a, b) => a - b);
    ...
    <div className={`${styles.journeysWrap} ${darkMode ? styles.journeysWrapDark : ''}`}>
          ...
          {Object.entries(CITY_POSITIONS).map(([city, pos]) => (
            <g key={city}>
              <circle
                ...
                cy={pos.y * H}
                r={3.5}
                fill={darkMode ? '#9ec1e655' : '#b8a07040'}
                stroke={darkMode ? '#9ec1e688' : '#b8a07080'}
                strokeWidth={0.8}
              />
              <text
                x={pos.x * W + 5}
              ...
              <g key={i}>
                {isLast && (
                  <circle
                    ...
                    cy={pos.y * H}
                    r={14}
                    fill={darkMode ? '#7aa8d833' : '#b8860b22'}
                    stroke={darkMode ? '#7aa8d8' : '#b8860b'}
                    strokeWidth={1}
                    strokeDasharray="3 2"
                  >
                ...
                <circle
                  ...
                  stroke={
                    isLast ? (darkMode ? '#d2e5f7' : '#7a5a00') : darkMode ? '#7aa8d8' : '#b8860b'
                  }
                ...
                <text
                  ...
                  fill={
                    isLast ? (darkMode ? '#e8f3ff' : '#3a2000') : darkMode ? '#c8def4' : '#6a5020'
                  }
                  fontFamily="serif"
                >
      ...
      <div className={styles.journeysControls}>
        <button
          ...
          onClick={() => {
            if (step >= path.length - 1) setStep(0);
            setPlaying(p => !p);
          }}
        >
