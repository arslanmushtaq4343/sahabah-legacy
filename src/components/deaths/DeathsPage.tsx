...
const MAP_LNG_MIN = 28,
  MAP_LNG_MAX = 62;
const MAP_LAT_MIN = 14,
  MAP_LAT_MAX = 42;
const MAP_W = 820,
  MAP_H = 440;
    ...
    <div className={`${s.page} premium-page`}>
        ...
        <div className={s.statsBar}>
          {(Object.keys(CAUSE_META) as DeathCause[]).map(k => (
            <div
              ...
              style={{ '--cc': CAUSE_META[k].color } as React.CSSProperties}
            >
      ...
      <div className={s.controls}>
        <div className={s.filterRow}>
          {CAUSES.map(c => (
            <button
              ...
              style={
                causeFilter === c.id && c.id !== 'all'
                  ? ({ '--ac': CAUSE_META[c.id as DeathCause].color } as React.CSSProperties)
                  : undefined
              }
            >
        ...
        <div className={s.viewToggle}>
          <button
            className={`${s.viewBtn} ${view === 'map' ? s.viewActive : ''}`}
            onClick={() => setView('map')}
          >
            🗺 Map
          </button>
                ...
                >
                  ...
                  {isSel && (
                    <text
                      ...
                      paintOrder="stroke"
                    >
                      {normalizeTransliteration(d.companion).split(' ').slice(0, 2).join(' ')}
                    </text>
                  )}
                </g>
              );
      ...
      {selected && <DetailPanel death={selected} onClose={() => setSelected(null)} />}
    ...
    <div className={s.panel} style={{ '--pc': meta.color } as React.CSSProperties}>
      <div className={s.panelHeader}>
        <div className={s.panelLeft}>
          ...
          <h2 className={s.panelName}>{normalizeTransliteration(d.companion)}</h2>
      ...
      <div className={s.panelMeta}>
        ...
        <div className={s.metaItem}>
          ...
          <span className={s.metaVal}>
            {d.location} · <span dir="rtl">{d.locationAr}</span>
          </span>
        </div>
        <div className={s.metaItem}>
      ...
      <div className={s.panelSection}>
        ...
        <p className={s.panelText}>{normalizeTransliteration(d.circumstance)}</p>
      </div>

      {d.lastMoments && (
        <div className={s.panelSection}>
          <span className={s.panelSectLabel}>Final Moments</span>
          <p className={s.panelText}>{normalizeTransliteration(d.lastMoments)}</p>
        </div>
      )}
      ...
      {d.propheticStatement && (
        <div className={s.propheticBox}>
          ...
          <p className={s.propheticText}>{normalizeTransliteration(d.propheticStatement)}</p>
        </div>
      )}
    </div>
  );
    ...
    <div className={`${s.listRow} ${isSelected ? s.listRowOpen : ''}`}>
      <button className={s.listSummary} onClick={onClick}>
        ...
        <div className={s.listNames}>
          <span className={s.listName}>{normalizeTransliteration(d.companion)}</span>
          <span className={s.listNameAr}>{d.companionAr}</span>
        </div>
        <div className={s.listMeta}>
          <span className={s.listYear}>
            {d.yearAH} AH · {d.yearCE} CE
          </span>
          <span className={s.listLoc}>{d.location}</span>
        </div>
        <span
          className={s.listCauseTag}
      ...
      {isSelected && (
        <div className={s.listDetail}>
          <p className={s.listCircumstance}>{normalizeTransliteration(d.circumstance)}</p>
          {d.lastMoments && (
            <div className={s.listLast}>
              <span className={s.listLastLabel}>Final Moments</span>
              <p className={s.listLastText}>{normalizeTransliteration(d.lastMoments)}</p>
            </div>
          )}
          {d.propheticStatement && (
            <div className={s.listProphetic}>
              <span className={s.listPropheticLabel}>Prophetic Statement</span>
              <p className={s.listPropheticText}>
                {normalizeTransliteration(d.propheticStatement)}
              </p>
            </div>
          )}
        </div>
