...
import {
  getHijriDayOfYear,
  DAILY_COMPANION_RANKS,
  DAILY_EVENTS,
  getStreak,
  STREAK_MILESTONES,
  getCurrentMilestone,
  getNextMilestone,
  getWeeklyCompanionRank,
  getVoteForCompanion,
  voteForCompanion,
  hasVoted,
  recordVote,
...
export function StudyStreakBar() {
  ...
  return (
    <>
        ...
        <div className={styles.streakInfo}>
          ...
          {next && (
            <span className={styles.streakNext}>
              {next.days - streak} days to {next.title}
            </span>
          )}
        </div>
        <div className={styles.streakProgressTrack}>
          <div
            ...
            style={{
              ...
              background: current?.color || '#d4a820',
            }}
          />
        </div>
        {current && (
          <span className={styles.streakMilestone} style={{ color: current.color }}>
            {current.title}
          </span>
        )}
      </div>
      ...
      {showDetail && (
        <div
          className={styles.streakOverlay}
          onClick={e => e.target === e.currentTarget && setShowDetail(false)}
        >
          <div className={styles.streakModal}>
            <button className={styles.streakClose} onClick={() => setShowDetail(false)}>
              ✕
            </button>
            ...
            <div className={styles.streakMilestones}>
              {STREAK_MILESTONES.map((m, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`${styles.smCard} ${reached ? styles.smReached : ''}`}
                    style={{ borderLeftColor: reached ? m.color : '#333' }}
                  >
                    <div className={styles.smHeader}>
                      <span className={styles.smBadge}>{m.badge}</span>
                      <span className={styles.smDays} style={{ color: reached ? m.color : '#666' }}>
                        {m.days} days
                      </span>
                      <span
                        className={styles.smTitle}
                        style={{ color: reached ? m.color : '#666' }}
                      >
                        {m.title}
                      </span>
                    ...
                    {reached && (
                      <>
                        <blockquote className={styles.smQuote} style={{ borderLeftColor: m.color }}>
                          {m.quote}
                        </blockquote>
                    ...
                    {!reached && (
                      <p className={styles.smLocked}>{m.days - streak} more days to unlock</p>
                    )}
                  </div>
                );
              })}
  ...
  return (
    <>
      {showDetail && companion && (
        <div
          className={styles.weeklyOverlay}
          onClick={e => e.target === e.currentTarget && setShowDetail(false)}
        >
          <div className={styles.weeklyModal} style={{ borderTopColor: color }}>
            <button className={styles.weeklyClose} onClick={() => setShowDetail(false)}>
              ✕
            </button>
            <div className={styles.weeklyModalHeader} style={{ background: color + '10' }}>
              ...
              <h3 className={styles.weeklyModalComp}>
                #{companion.rank} — {companion.name}
              </h3>
            </div>
            <div className={styles.weeklyContent}>
              {companion.description && (
                <p className={styles.weeklyDesc}>{companion.description}</p>
              )}
              {companion.significance && (
                <p className={styles.weeklySig}>{companion.significance}</p>
              )}
              {companion.legacy && (
              ...
              <div className={styles.weeklyVote}>
                ...
                <button
                  className={styles.weeklyVoteBtn}
                  style={{ background: voted ? '#333' : color, opacity: voted ? 0.7 : 1 }}
                  onClick={handleVote}
                  disabled={voted}
                >
  ...
  const options = [
    {
      key: 'highContrast',
      label: 'High Contrast',
      desc: 'WCAG AAA compliant — increases all contrast ratios for visibility',
    },
    {
      key: 'largeFonts',
      label: 'Large Fonts',
      desc: 'Increases all font sizes by 20% for easier reading',
    },
    { key: 'reduceMotion', label: 'Reduce Motion', desc: 'Removes all animations and transitions' },
    {
      key: 'dataTableMode',
      label: 'Table Mode',
      desc: 'Replaces SVG visualizations with accessible data tables',
    },
  ];
        ...
        <div className={styles.a11yPanel} role="dialog" aria-label="Accessibility Settings">
          ...
          {options.map(opt => (
            <label key={opt.key} className={styles.a11yOption}>
              ...
              <div
                className={`${styles.a11yToggleSwitch} ${settings[opt.key] ? styles.a11yOn : ''}`}
